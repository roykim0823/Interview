...
Solutions:
	A. Use fixed size
	B. Flexible Size
		- Each stack has data and an index (pointer) for previous stack element
*/
...
// val= data, preIdx = previous index of the stack
typedef struct node{
    int val,preIdx;
}node;

// Flexible stack size with each item keep track of array index for each stack
// Stack has a pointer to point the previous element
class stack3_1{
public:
    stack3_1(int totalSize = 300){
    ...
    void pop(int stackNum){
		// stack index = previous stack index!
