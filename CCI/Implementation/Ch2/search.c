...
/* search.c -- test and time binary and sequential search
   ...
   3.) Time all functions
		...
		Output lines: algnum n numtests clock_ticks nanosecs_per_elem
...
int binsearch(DataType t, int l, int u)
{
   int m;
   if (l > u) return -1;                     /* base case: key not in table */ 
   assert( x[l] <= t && t <= x[u] );         /* x[] should be ascending. */
   m = (l + u) / 2;
   if (x[m] ==t) return m;                   /* base case: key in table */ 
   if (x[m] > t) return binsearch(t, l, m-1);  /* inductive case: left  */
   if (x[m] < t) return binsearch(t, m+1, u);  /* inductive case: right */
}

int inductive_binarysearch(DataType t)
{
    int l = 0;
    int u = n-1;
    return binsearch(t, l, u);
}
...
void timedriver()
{	int i, algnum, numtests, test, start, clock_ticks;
	printf("%s\t%s\t%s\t%s (%d/sec)\t%s\n",
		"alg#", "n", "numtests", "clock_ticks", CLOCKS_PER_SEC, "nanosec/elem");
	while (scanf("%d %d %d", &algnum, &n, &numtests) != EOF) {
		for (i = 0; i < n; i++)
			x[i] = i;
		for (i = 0; i < n; i++)
			p[i] = i;
		scramble(n);        /* generates p = a random permutation of size n */
		start = clock();
		for (test = 0; test < numtests; test++) {
			for (i = 0; i < n; i++) {
				...
				case 8: assert(inductive_binarysearch(p[i]) == p[i]); break;
		...
		clock_ticks = clock() - start;
		printf("alg %d\t%d\t%d\t%d\t%g\n",
			algnum, n, numtests, clock_ticks,
			1e9*clock_ticks/((float) CLOCKS_PER_SEC*n*numtests));
	}
}
