...
IntSetList random_set(int m, int maxval)
{	int *v = new int[m];
	IntSetList S(m, maxval);
	while (S.size() < m)
		S.insert(bigrand() % maxval);
	S.report(v);
	cout << "size of random set: " << m << "\n";
	cout << "maxvalue (modulus): " << maxval << "\n";
	cout << "elements in random set v:\n";
  	for (int i = 0; i < m; i++)
		cout << v[i] << "\n";
	return S;
}
...
{	int maxelements = atoi(argv[1]);
	...
	// create sets using about 10 different underlying data structures
	IntSetList   List_Set(maxelements, maxval);
	IntSetList2  List2_Set(maxelements, maxval);
	IntSetArr    Arr_Set(maxelements, maxval);
	IntSetSTL    STL_Set(maxelements, maxval);
	IntSetBitVec Bit_Set(maxelements, maxval);
	IntSetBST    BST_Set(maxelements, maxval);
	IntSetBST2   BST2_Set(maxelements, maxval);
	IntSetBins   Bins_Set(maxelements, maxval);
	IntSetBins2  Bins2_Set(maxelements, maxval);

	IntSetList   Rand_Set = random_set(maxelements, maxval);
	// overheadonly(maxelements, maxval);
	// memaccesstest(maxelements, maxval);
	return 0;
}
