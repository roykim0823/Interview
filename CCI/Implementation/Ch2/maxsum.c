...
#define max(a, b) max_function(a, b)
...
{   int i;
    ...
    for (i = 0; i < n; i++) {
        maxendinghere += x[i];
        maxendinghere = max_macro(maxendinghere, 0);
       	maxsofar = max_macro(maxsofar, maxendinghere);
    }
    return maxsofar;
}
...
{   int i;
    ...
    for (i = 0; i < n; i++) {
        maxendinghere += x[i];
        maxendinghere = max_function(maxendinghere, 0);
       	maxsofar = max_function(maxsofar, maxendinghere);
    }
    return maxsofar;
}
...
{   int algnum, start, clicks;
    ...
    printf("%s\t%s\t%s\t%s (%d/sec)\t%s\n",
        "alg#", "n", "max so far", "clock_ticks", CLOCKS_PER_SEC, "sec");
