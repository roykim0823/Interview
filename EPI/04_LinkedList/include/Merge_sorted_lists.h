...
template <typename T>
void append_node(shared_ptr<ListNode<T>>& head, shared_ptr<ListNode<T>>& tail,
                 shared_ptr<ListNode<T>>& n) {
...
// @include
template <typename T>
shared_ptr<ListNode<T>> merge_sorted_linked_lists(
  shared_ptr<ListNode<T>> F, shared_ptr<ListNode<T>> L) {
  
  shared_ptr<ListNode<T>> sorted_head = nullptr, tail = nullptr;
