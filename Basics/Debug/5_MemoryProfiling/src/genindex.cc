...
void FileIndexType::print_index() {
	...
	for(it = wordlindex.begin(); it != wordlindex.end(); ++it) {
...
int main(int argc, char *argv) {
	...
	for(i=0; i<argc-1; i++) {
		result = fileindex[i].scan_file(argv[i+1]_;
		if(result)
			return result;	// something went wrong with file read
		result = fileindex[i].verify_index();
		if(result) {
			fprintf(stderr, "-- error: index verification failed.\n");
			return result;
		}
#ifdef CLEAR_INDEX
...
#endif
	fprintf(stderr, "-- memoroy size, all data structures: %d bytes\n",
			total);
	return result;
}
